...
In this appendix, we provide a variety of examples of assessments.}
...
\marginnote{\added{XX add commentary here...}}
...
\marginnote{\added{Sample solution: For an observational study which assessed the association between coffee drinking and cancer, smoking status could mask (or ``confound") the relationship, since smoking could be associated with both coffee drinking and cancer.}}%
...
\textbf{\textit{Assessment items to avoid using on tests: True/False, pure computation without a context or interpretation, items with too much data to enter and compute or analyze, \added{or} items that only test memorization of definitions or formulas.}}
...
It was claimed 
\marginnote{\added{Critique: This problem requires use of software to calculate the exact binomial or use of the binomial approximation to the normal.  Computer output might be provided to augment this question and facilitate solution.}}
...
\replaced{Francisco Franco (Class of '98)}{Francis (Class of '18)} weighed 100 Hershey's Kisses (with almonds).  She found that the sample average was 4.80 grams and the SD was .28 grams. \textit{In the context of this setting,} explain what is meant by the sampling distribution of an average.
...
\marginnote{The list of methods or examples can be shortened.}
\begin{enumerate} [leftmargin=1cm, itemsep=.2em]
...
\marginnote{\added{XX Provide the t-value?  Make the sample sizes equal?  Provide the pooled standard error as well?  Provide a graphical display of the results of a permutation test?}}
...
\marginnote{\added{XX Candies as motivation?  Should we note that we need to move beyond M\&M's?}}
\begin{verbatim}
Kate:                Dana:                Minh:
...
If needed, we converted the competencies into a numeric score. 
Often, we begin by giving a score of 85 for achieving basic competency in all 5 categories.
Then we add to this score for competencies that surpass the basic level and subtract for 
those that need improvement.
Typically, we add (subtract) 3 points for each of the first 3 competencies that 
have surpassed the basic (need improvement), then
we add (subtract) 4 points for the fourth competency that is surpassed (needs improvement) 
and 5 points for the fifth competency. 
