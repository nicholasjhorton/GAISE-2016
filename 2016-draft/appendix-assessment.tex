...
\marginnote{\textit{Sample solution: For an observational study which assessed the association between coffee drinking and cancer, smoking status could mask (or ``confound") the relationship, since smoking could be associated with both coffee drinking and cancer \added{(see also Appendix E,
...
A study found that individuals who lived in houses with more than two bathrooms tended to have higher blood pressure than individuals who lived in houses with two or fewer bathrooms.  Can cause-and-effect be determined from this?

\subsection{\textbf{\textit{Item 16}}}
Researchers took random samples of subjects from two populations and applied a test to the data; the \textit{P}-value for the test, using a nondirectional alternative, was .06.  For each of the following, say whether the statement is true or false and why.
\begin{enumerate}[leftmargin=1cm, itemsep=.2em]
...
\added{As the name suggests, the Old Faithful geyser in Yellowstone National
...
\noindent
\added{You are a busy tourist and have only 10 minutes to sit around and
...
\added{You can easily ask the ranger what was the duration of the previous
eruption. 
...
A student weighed 100 \added{industrial diamonds}.  She found that the sample average weight was 4.80 grams and the SD was .28 grams. \textit{In the context of this setting,} explain what is meant by the sampling distribution of an average.
...
A student collected data on a random sample of 12 breakfast cereals.  \replaced{They}{He} recorded x = fiber (in grams/ounce) and y = price (in cents/ounce).  A scatterplot of the data shows a linear relationship.  The fitted regression model is
